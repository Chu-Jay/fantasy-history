...
function calculateSeasons(){const map=new Map;for(const x of ledger.filter(x=>!x.week.startsWith('P'))){const k=`${x.year}|${x.owner}`;if(!map.has(k))map.set(k,{year:x.year,owner:x.owner,team:x.team,score:0});map.get(k).score+=x.score}return [...map.values()]}
