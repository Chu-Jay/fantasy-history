...
function consistencyGrade(v,all){const q=p=>percentile(all,p);if(v>=q(.99))return'S';if(v>q(.95))return'A+';if(v>=q(.9))return'A';if(v>=q(.85))return'A-';if(v>=q(.8))return'B+';if(v>=q(.7))return'B';if(v>=q(.6))return'B-';if(v>=q(.5))return'C+';if(v>=q(.4))return'C';if(v>=q(.3))return'C-';if(v>=q(.1))return'D';return'D'}
function dynastyStats(owner){const all=ledger.filter(x=>x.owner===owner),reg=all.filter(x=>/^\d+$/.test(x.week)),play=all.filter(x=>x.week.startsWith('P'));
 ...
 const svals=all.map(x=>x.seasonPoints).filter(x=>x>0),uvals=all.map(x=>x.seasonFinalPoints).filter(x=>x>0), volatility=(svals.length?Math.max(...svals):0)+(uvals.length?Math.min(...uvals):0);
 const playoffYears=new Set(play.map(x=>x.year)).size, playoffWins=play.filter(x=>x.outcome==='w').length, finals=all.filter(x=>x.week==='P Finals'), finalApps=finals.length, titles=finals.filter(x=>x.outcome==='w').length;
 const avgLik=all.length?all.reduce((a,x)=>a+x.likelihood,0)/all.length:0, avgOpp=all.length?all.reduce((a,x)=>a+x.oppLikelihood,0)/all.length:0, luck=totalPct-avgLik;
 const cons=reg.length?reg.reduce((a,x)=>a+x.consistency,0)/reg.length:0;
...
function setupDynastyPage(){const box=document.querySelector('#includeDefunct');box.checked=localStorage.getItem('ror-dynasty-defunct')==='1';box.onchange=()=>{localStorage.setItem('ror-dynasty-defunct',box.checked?'1':'0');renderDynasty()};renderDynasty()}
function renderDynasty(){const latestYear=[...new Set(ledger.map(x=>x.year))].sort((a,b)=>+b-+a)[0],current=new Set(ledger.filter(x=>x.year===latestYear).map(x=>x.owner)),allOwners=[...new Set(ledger.map(x=>x.owner))],owners=document.querySelector('#includeDefunct').checked?allOwners:allOwners.filter(x=>current.has(x));let rows=owners.map(dynastyStats);const allCons=rows.map(x=>x.cons);rows.forEach(x=>x.grade=consistencyGrade(x.cons,allCons));rows.sort((a,b)=>b.prestige-a.prestige);rows.forEach((x,i)=>x.rank=i+1);
 const top=rows.slice(0,3);document.querySelector('#dynastyPodium').innerHTML=top.map(x=>`<article class="dynasty-podium"><span class="dynasty-rank">#${x.rank}</span>${identityHTML(x.year,x.owner,x.team,'history-logo')}<strong>${fmt(x.prestige)}</strong><small>Prestige</small></article>`).join('');
 const head='<tr><th>Rank</th><th>Manager</th><th>Prestige</th><th>Total Win %</th><th>Matchup Win %</th><th>Median Win %</th><th>PPW</th><th>Season Volatility</th><th>Playoff Apps</th><th>Playoff Wins</th><th>Finals Apps</th><th>Titles</th><th>Avg Likelihood</th><th>Opp Likelihood</th><th>Luck</th><th>Consistency</th><th>Grade</th></tr>';
 document.querySelector('#dynastyTable').innerHTML=`<thead>${head}</thead><tbody>${rows.map(x=>`<tr><td><strong>#${x.rank}</strong></td><td>${identityHTML(x.year,x.owner,x.team,'stand-logo')}</td><td><strong>${fmt(x.prestige)}</strong></td><td>${(x.totalPct*100).toFixed(1)}%</td><td>${(x.matchupPct*100).toFixed(1)}%</td><td>${(x.medianPct*100).toFixed(1)}%</td><td>${fmt(x.ppw)}</td><td>${fmt(x.volatility)}</td><td>${x.playoffYears}</td><td>${x.playoffWins}</td><td>${x.finalApps}</td><td>${x.titles}</td><td>${(x.avgLik*100).toFixed(1)}%</td><td>${(x.avgOpp*100).toFixed(1)}%</td><td>${x.luck>=0?'+':''}${(x.luck*100).toFixed(1)}%</td><td>${x.cons.toFixed(2)}</td><td><strong>${x.grade}</strong></td></tr>`).join('')}</tbody>`;
 document.querySelector('#dynastyCount').textContent=`Comparing ${rows.length} manager${rows.length===1?'':'s'} • ${document.querySelector('#includeDefunct').checked?'All historical teams':'Current teams only'}`}
load();
