...
const FALLBACK_LOGO='league-logo.png';
async function load(){try{const [lt,st,ht]=await Promise.all(Object.values(URLS).map(u=>fetch(u,{cache:'no-store'}).then(r=>{if(!r.ok)throw Error(r.status);return r.text()})));const lr=csv(lt),sr=csv(st),hr=csv(ht);ledger=lr.slice(1).filter(r=>r[0]&&r[3]&&r[4]).map(r=>({owner:r[0].trim(),score:num(r[1]),opponent:r[2].trim(),year:String(r[3]).replace(/\.0$/,''),week:r[4].trim(),team:(r[5]||r[0]).trim(),oppScore:num(r[6]),outcome:(r[7]||'').trim().toLowerCase(),diff:num(r[12])}));renderLeague(sr);parseTeams(sr);parseChampions(hr);renderChampion();renderShrine();setupSelectors();document.querySelector('#status').textContent='Live data loaded'}catch(e){document.querySelector('#status').textContent='Could not load live data';console.error(e)}}
...
function logoAttrs(src){return `src=\"${esc(src||FALLBACK_LOGO)}\" onerror=\"this.onerror=null;this.src='${FALLBACK_LOGO}'\"`}
