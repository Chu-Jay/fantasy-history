...
function dynastyGrade(v,all){const q=p=>percentile(all,p);if(v>=q(.99))return'S';if(v>q(.95))return'A+';if(v>=q(.9))return'A';if(v>=q(.85))return'A-';if(v>=q(.8))return'B+';if(v>=q(.7))return'B';if(v>=q(.6))return'B-';if(v>=q(.5))return'C+';if(v>=q(.4))return'C';if(v>=q(.3))return'C-';return'D'}
...
function renderDynasty(){const latestYear=[...new Set(ledger.map(x=>x.year))].sort((a,b)=>+b-+a)[0],current=new Set(ledger.filter(x=>x.year===latestYear).map(x=>x.owner)),allOwners=[...new Set(ledger.map(x=>x.owner))],owners=document.querySelector('#includeDefunct').checked?allOwners:allOwners.filter(x=>current.has(x));let rows=owners.map(dynastyStats);const allPrestige=rows.map(x=>x.prestige);rows.forEach(x=>x.grade=dynastyGrade(x.prestige,allPrestige));const sortBy=document.querySelector('#dynastySort').value;rows.sort((a,b)=>sortBy==='consistency'?(b.cons-a.cons||b.prestige-a.prestige):(b.prestige-a.prestige||b.cons-a.cons));rows.forEach((x,i)=>x.rank=i+1);
 ...
 document.querySelector('#dynastyTable').innerHTML=`<thead>${head}</thead><tbody>${rows.map(x=>`<tr><td><strong>#${x.rank}</strong></td><td>${identityHTML(x.year,x.owner,x.team,'stand-logo')}</td><td><strong>${fmt(x.prestige)}</strong></td><td><strong>${x.cons.toFixed(2)}</strong></td><td><strong>${x.grade}</strong></td><td>${(x.totalPct*100).toFixed(1)}%</td><td>${(x.matchupPct*100).toFixed(1)}%</td><td>${(x.medianPct*100).toFixed(1)}%</td><td>${fmt(x.ppw)}</td><td>${fmt(x.volatility)}</td><td>${x.playoffYears}</td><td>${x.playoffWins}</td><td>${x.finalApps}</td><td>${x.titles}</td><td>${(x.avgLik*100).toFixed(1)}%</td><td>${(x.avgOpp*100).toFixed(1)}%</td><td>${x.luck>=0?'+':''}${(x.luck*100).toFixed(1)}%</td></tr>`).join('')}</tbody>`;
